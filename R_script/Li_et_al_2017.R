...
# FigS4 FT and DC correlation ---------------------------------------------


FT_DC_original <- read.csv(file= "July2016_12s_onestep_filtration/Reanalysis_May_2017/AppendixS2_Filtration_July_2016_FT_DC.csv", header = TRUE)


FT_DC_original$Pond <-mapvalues(FT_DC_original$Pond, c("E1","E2","E3","E4"), 
                                           c("A","B","C","D"))

##correlation between filtration time and DNA concentration, two outlier were detected T3-1-3 and T2-2-3
ggplot(FT_DC_original,aes(y=DNA,x=Time,colour=Treatment))+
...
ggsave("FigS4_FT_DC.jpeg",path = "July2016_12s_onestep_filtration/Reanalysis_May_2017/Figures/", width = 12, height = 9, units = "in", dpi=500)
...
FT_Statistic$y <- c(rep(50,6),rep(25,6),rep(50,6),rep(20,6))

FT_boxplot+geom_text(data=FT_Statistic,aes(Treatment,y, label=Letter, fill=NA),hjust = 0.5, size=7, parse = TRUE)



ggsave("Fig2_FT_boxplot.jpeg",path = "July2016_12s_onestep_filtration/Reanalysis_May_2017/Figures/", width = 12, height = 9, units = "in", dpi=500)
...
FT_Treament_ST$y <- c(rep(80,4),rep(55,4),rep(20,4),rep(110,4),rep(20,4),rep(4,4))

FT_Treatment_boxplot+geom_text(data=FT_Treament_ST,aes(Pond,y, label=Letter, fill=NA),hjust = 0.5, size=7, parse = TRUE)

ggsave("FigS7_FT_treat.jpeg",path = "July2016_12s_onestep_filtration/Reanalysis_May_2017/Figures/", width = 12, height = 9, units = "in", dpi=500)
...
ggsave("Fig3_DC_boxplot.jpeg",path = "July2016_12s_onestep_filtration/Reanalysis_May_2017/Figures/", width = 12, height = 9, units = "in", dpi=500)
...
ggsave("FigS6_abundance_rep.jpeg",path = "July2016_12s_onestep_filtration/Reanalysis_May_2017/Figures/", width = 10, height = 12, units = "in", dpi=500)
...
# FigS8_heterogeneity_rep_boxplot_outliers -------------------------------------------------------------------
...
ggplot(Filtration_rs_sample_outlier,aes(x=Treatment,y=Percent_reads))+
  ...
  theme(text=element_text(size=20),axis.text.x = element_text(angle = -15,vjust = 0.7),legend.title = element_text(size=15),legend.text = element_text(size=15))



ggsave("FigS8_heterogeneity_rep_boxplot_outliers.jpeg",path = "July2016_12s_onestep_filtration/Reanalysis_May_2017/Figures/", width = 12, height = 9, units = "in", dpi=500)
...
Horn_NMDS<-grid.arrange(spadeR_all_boxplot, Pond_NMDS_0.5,ncol = 1, nrow = 2,
            heights = c(3.0,3.5))



ggsave("Fig5_Horn_NMDS_0.5.jpeg",Horn_NMDS,path = "July2016_12s_onestep_filtration/Reanalysis_May_2017/Figures/", width = 12, height = 17, units = "in", dpi=500)
...
ggplot(Pond_summary, aes(x= Date, y= Number,colour=Species,group=Species))+
  ...
  labs(x="Month", y="Fish number")+theme_bw()+
...
ggplot(Merge_Filtration_sub_bio_num,aes(x=Treatment,y=Percent_reads,fill=Species))+
  ...
  theme(text=element_text(size=20),axis.text.x = element_text(angle = -25,vjust = 0.6),legend.title = element_text(size=15),legend.text = element_text(size=15))



ggsave("Fig4_relative abundance_bio_num.jpeg",path = "July2016_12s_onestep_filtration/Reanalysis_May_2017/Figures/", width = 12, height = 9, units = "in", dpi=500)
...
Corr_fishnum <- ddply(Filtration_summary, .(Pond,Treatment), summarise, s=corfun(lgNum,lgReads)$statistic,
                cor.est=corfun(lgNum,lgReads)$estimate,
                pval=corfun(lgNum,lgReads)$p.value,
                alt=corfun(lgNum,lgReads)$alternative,
                df=corfun(lgNum,lgReads)$parameter)
...
Filtration4_log_bio_reads<- ddply(Filtration_summary, .(Treatment), summarise, s=corfun(lgBio,lgReads)$statistic,
...
Corr_fishbio <-ddply(Filtration_summary, .(Pond,Treatment), summarise, s=corfun(lgBio,lgReads)$statistic,
                cor.est=corfun(lgBio,lgReads)$estimate,
                pval=corfun(lgBio,lgReads)$p.value,
                alt=corfun(lgBio,lgReads)$alternative,
                df=corfun(lgBio,lgReads)$parameter)
...
ggsave("FigS9_Lg_Bio_reads.jpeg",path ="July2016_12s_onestep_filtration/Reanalysis_May_2017/Figures/", width = 12, height = 9, units = "in", dpi=500)
